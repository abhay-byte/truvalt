...
truvalt addresses this gap by providing a secure, fully offline-capable password manager that works natively on Android with optional cloud sync via Firebase. It offers zero-knowledge end-to-end encryption, no mandatory subscription, and a first-class native Android UI.

---

## The Specific Problem Being Solved

Users need a secure, fully offline-capable password manager that works natively on Android — with optional Firebase cloud sync, zero-knowledge end-to-end encryption, no mandatory subscription, and a first-class native UI.
...
- [x] Build a native Android password manager
- [x] Implement zero-knowledge end-to-end encryption (AES-256-GCM + Argon2id)
- [x] Support both Firebase cloud sync and local-only offline mode
...
- **Master Password Constraint:** Master password is NEVER sent to any server; only a derived authentication key is used locally
- **Zero-Knowledge Architecture:** Cloud store (Firestore) holds only encrypted blobs and metadata
- **Network Security:** HTTPS only via Firebase SDK (TLS 1.2+)
- **Encryption:** AES-256-GCM for vault items, Argon2id for key derivation
- **Platform:** Android (Kotlin + Compose)
- **Database:** Room (Android local vault), Cloud Firestore (cloud sync)
- **Auth Methods:** Email + Password (Firebase Auth), TOTP 2FA, Passkey (WebAuthn), Biometric (Android)
